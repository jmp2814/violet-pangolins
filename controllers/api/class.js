...
router.get('/:id', async (req, res) => {
    try{
      ...
      if (!categoryData) {
        res.status(404).json({ message: `No classroom found with ID: ${req.params.id}` });
        return;
      }
...
router.post('/', async (req, res) => {
  ...
  router.put('/:id', async (req, res) => {
    try{
      ...
      if (!classroomData) {
        res.status(404).json({ message: `No classrom found with ID: ${req.params.id}` });
        return;
      }
    
      if (!updateclassroomData[0]) {
        res.status(400).json({ message: `No classroom data updated for ID: ${req.params.id}` });
        return;
      };
